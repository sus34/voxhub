...
**Вход:** личный аккаунт; новые люди приходят по ссылке-приглашению от владельца сервера.
...
- **Аккаунты и приглашения:** кто настроил сервер — владелец. Он раздаёт ссылки
  со сроком и лимитом использований, выгоняет и банит. Пароли — scrypt, сессия —
  httpOnly-cookie, которую сервер может отозвать; данные в одном файле SQLite
- Каналы — переход только по явной кнопке, случайным кликом из разговора не выкинет
...
- Громкость каждого человека до 200% — запоминается между запусками
...
На пустой базе сервер печатает в лог **код настройки** — с ним на первом экране создаётся
сервер и владелец. База лежит в `DATA_DIR` (по умолчанию `server/data/voxhub.db`).

```bash
cd server && npm test        # тесты API: настройка, вход, приглашения, роли
```
...
- **Забытый пароль** пока не сбросить — владелец выдаёт новое приглашение
  ([#28](https://github.com/sus34/voxhub/issues/28)).
...
[AGPL-3.0](LICENSE): ставь себе, меняй, делись. Если запускаешь изменённую версию как
сервис для других — исходники изменений тоже открываешь.
