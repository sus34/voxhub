...
3. **`scalabilityMode: 'L1T3'`** — один слой разрешения, три временных. Дефолтный `L3T3\_KEY`
...
Но главное здесь — **`restrictOwnAudio: true`** в `SCREEN\_AUDIO\_CAPTURE`. Захват системного
...
Адрес сервера берётся из `VOXHUB\_URL` — это адрес компьютера того, кто держит комнату.
...
Версия в `package.json` должна совпадать с `DESKTOP\_VERSION`, иначе приложение
...
* **Лимит 5 участников** зашит в `deploy/livekit.yaml` (`max\_participants`).
* **Нет записи** стримов.
